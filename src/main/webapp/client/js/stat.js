...
function createTable(data){
    ...
    for(var i =0 ;i<length;i++) {
        var rowTemplate = "<tr id=" + i + "><td class='cnname'>%s</td><td class='mainNet'>%s</td><td class='backupNet'>%s</td>"
            + "<td class='gateway'>%s</td><td class='switcher'>%s</td><td class='host'>%s</td></tr>";
        var row = sprintf(rowTemplate, jsonArray[i].cnname, jsonArray[i].mainNet, jsonArray[i].backupNet, judge(jsonArray[i].gateway), judge(jsonArray[i].switcher), judge(jsonArray[i].host));
        $("#state").append($(row));
    }
    replaceTr($("#state"));
    setCss();
