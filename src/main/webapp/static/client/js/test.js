...
function createTable(data){
    ...
    for(var i =0 ;i<length;i++) {
        var rowTemplate = "<tr id=" + i + "><td class='city'>%s</td><td class='main'>%s</td><td class='backup1'>%s</td>"
            + "<td class='backup2'>%s</td><td class='mirror'>%s</td></tr>";
        var row = sprintf(rowTemplate, jsonArray[i].city, jsonArray[i].main, jsonArray[i].backup1, jsonArray[i].backup2, jsonArray[i].mirror);
        $("#state").append($(row));
    }
    replaceTr($("#state"));
    setCss();
