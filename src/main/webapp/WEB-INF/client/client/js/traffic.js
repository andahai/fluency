...
var screenFlag = 0;
function init(){
    var url="/nms/getInterfaceTraffic";
    $.get(url,function(data){
        createInterfaceTable(data);
    });
    url = "/nms/getStationTraffic";
    $.get(url,function(data){
...
function createInterfaceTable(data){
    ...
    for(var i =0 ;i<length;i++) {
        ...
        var row = sprintf(rowTemplate, jsonArray[i].name, judge(jsonArray[i].state), judge(jsonArray[i].pps_recv), judge(jsonArray[i].pps_send), judge(jsonArray[i].bps_recv),
             judge(jsonArray[i].bps_send),judge(jsonArray[i].packets_recv),judge(jsonArray[i].packets_send),judge(jsonArray[i].bytes_recv),judge(jsonArray[i].bytes_send));
...
function createStationTable(data){
    ...
    var length = jsonArray.length;
    for(var i =0 ;i<length;i++) {
        var rowTemplate = "<tr id=station" + i + "><td class='cnname'>%s</td><td class='mainRecvSpeed'>%s</td><td class='mainSendSpeed'>%s</td>"
            + "<td class='backupRecvSpeed'>%s</td><td class='backupSendSpeed'>%s</td><td class='mainRecvTotal'>%s</td><td class='mainSendTotal'>%s</td>"
            +"<td class='backupRecvTotal'>%s</td><td class='backupSendTotal'>%s</td><td class='recvLostRate'>%s</td><td class='sendLostRate'>%s</td></tr>";
        var row = sprintf(rowTemplate, jsonArray[i].cnname, judge(jsonArray[i].mainRecvSpeed), judge(jsonArray[i].mainSendSpeed), judge(jsonArray[i].backupRecvSpeed), judge(jsonArray[i].backupSendSpeed),
             judge(jsonArray[i].mainRecvTotal),judge(jsonArray[i].mainSendTotal),judge(jsonArray[i].backupRecvTotal),judge(jsonArray[i].backupSendTotal),judgePercent(jsonArray[i].recvLostRate),judgePercent(jsonArray[i].sendLostRate));

        $("#stationTraffic").append($(row));
    }
    setCss();
}
//轮询接口信息
function ajax_data(){
    ...
    $.get(url,function(data)
    {
        ...
        for(var i = 0;i<length;i++){
            for(var k in jsonArray[i]){
                $('#station'+i).children('.'+k).text(judge(jsonArray[i][k]));
            }
        }
    });
}
